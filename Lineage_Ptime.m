function [Lineage, Ptime,CC_adjacent,Tree] = Lineage_Ptime(W,No_cluster,cluster_label,root_cluster,root_cell,latent,reverse)
...
%         CC_adjacent(i,j) = sum(sub_matrix(:))./(p.*q);
        CC_adjacent(i,j) = sum(sub_matrix(:))./nnz(sub_matrix);
    end
end
...
% [~,Ptime] = sort(Short_pathd(root_cell,:));
% % 
